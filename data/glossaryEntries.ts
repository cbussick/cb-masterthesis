import { CBGlossaryEntry } from "@/components/CBGlossary/CBGlossaryEntry";
import { CBTopic } from "./topics";

export const glossaryEntries: CBGlossaryEntry[] = [
  {
    id: "zellkern",
    term: "Zellkern",
    definition:
      "Der Zellkern ist das membranumhüllte Zellorganell, das das genetische Material (DNA) enthält und für die Steuerung der zellulären Aktivitäten verantwortlich ist.",
    unlockLevel: 1,
    image: "/cell-organelles/zellkern.png",
    image1x1: "/cell-organelles/zellkern_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "mitochondrien",
    term: "Mitochondrien",
    definition:
      "Mitochondrien sind Zellorganellen, die als Kraftwerke der Zelle dienen und Energie in Form von ATP durch Zellatmung produzieren.",
    unlockLevel: 1,
    image: "/cell-organelles/mitochondrium.png",
    image1x1: "/cell-organelles/mitochondrium_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "chloroplasten",
    term: "Chloroplasten",
    definition:
      "Chloroplasten sind für pflanzliche Zellen spezifische Zellorganellen und sind für die Photosynthese verantwortlich. Sie enthalten den Farbstoff Chlorophyll, das Lichtenergie einfängt und in chemische Energie umwandelt.",
    unlockLevel: 1,
    image: "/cell-organelles/chloroplast.png",
    image1x1: "/cell-organelles/chloroplast_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "endoplasmatisches retikulum",
    term: "Endoplasmatisches Retikulum (ER)",
    definition:
      "Das Zellorganell endoplasmatisches Retikulum ist ein membranumhülltes Netzwerk von Kanälen und Röhren, das eine Vielzahl von Funktionen erfüllt, einschließlich der Proteinsynthese, Lipid- und Hormonsynthese sowie des Transports von Stoffen innerhalb der Zelle.",
    unlockLevel: 1,
    image: "/cell-organelles/endoplasmatisches-retikulum.png",
    image1x1: "/cell-organelles/endoplasmatisches-retikulum_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "golgi-apparat",
    term: "Golgi-Apparat",
    definition:
      "Der Golgi-Apparat ist ein Zellorganell und für die Modifikation, Sortierung und den Transport von Proteinen und Lipiden innerhalb der Zelle zuständig. Er besteht aus Stapeln flacher Membransäckchen.",
    unlockLevel: 1,
    image: "/cell-organelles/golgi-apparat.png",
    image1x1: "/cell-organelles/golgi-apparat_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "lysosomen",
    term: "Lysosomen",
    definition:
      "Lysosomen sind membranumhüllte Zellorganellen, die Verdauungsenzyme enthalten und für den Abbau von Makromolekülen, abgenutzten Organellen und Fremdstoffen verantwortlich sind.",
    unlockLevel: 1,
    image: "/cell-organelles/lysosomen.png",
    image1x1: "/cell-organelles/lysosomen_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "vesikel",
    term: "Vesikel",
    definition:
      "Vesikel sind kleine membranumhüllte Strukturen, die den Transport von Stoffen innerhalb der Zelle ermöglichen.",
    unlockLevel: 1,
    image: "/cell-organelles/vesikel.png",
    image1x1: "/cell-organelles/vesikel_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "plasmamembran",
    term: "Plasmamembran",
    definition:
      "Die Plasmamembran grenzt das Zellinnere vom -äußeren ab und reguliert den Ein- und Austritt von Stoffen.",
    unlockLevel: 1,
    image: "/cell-organelles/plasmamembran.png",
    image1x1: "/cell-organelles/plasmamembran_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "peroxisomen",
    term: "Peroxisomen",
    definition:
      "Peroxisomen sind membranumhüllte Zellorganellen, die an verschiedenen Stoffwechselprozessen beteiligt sind, einschließlich dem Abbau von Fettsäuren und der Entgiftung von schädlichen Verbindungen.",
    unlockLevel: 1,
    image: "/cell-organelles/peroxisome.png",
    image1x1: "/cell-organelles/peroxisome_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "ribosomen",
    term: "Ribosomen",
    definition:
      "Ribosomen sind Zellorganellen, die an der Proteinbiosynthese beteiligt sind und entweder im Zytoplasma oder an der Oberfläche des endoplasmatischen Retikulums vorkommen.",
    unlockLevel: 1,
    image: "/cell-organelles/ribosom.png",
    image1x1: "/cell-organelles/ribosom_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "mikrotubuli",
    term: "Mikrotubuli",
    definition:
      "Mikrotubuli sind Teil des Cytoskeletts und bestehen aus dem Protein Tubulin. Sie sind wichtig für die Zellstruktur, die Zellteilung, den Transport von Organellen und den Aufbau von Zellfortsätzen wie Zilien und Flagellen.",
    unlockLevel: 1,
    image: "/cell-organelles/mikrotubuli.png",
    image1x1: "/cell-organelles/mikrotubuli_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "mikrofilamente",
    term: "Mikrofilamente",
    definition:
      "Mikrofilamente sind ebenfalls Teil des Cytoskeletts und bestehen aus dem Protein Aktin. Sie sind für die Zellstruktur, die Zellbewegung, die Kontraktion von Muskelzellen und die Bildung von Zellfortsätzen wie Mikrovilli verantwortlich.",
    unlockLevel: 1,
    topic: CBTopic.Zelle,
  },
  {
    id: "vakuolen",
    term: "Vakuolen",
    definition:
      "Vakuolen sind große, flüssigkeitsgefüllte Vesikel in pflanzlichen Zellen. Sie sind für die Speicherung von Wasser, Nährstoffen, Abfallstoffen und Pigmenten verantwortlich. Vakuolen spielen auch eine Rolle bei der Aufrechterhaltung des osmotischen Drucks und der Stabilität der Zelle.",
    unlockLevel: 1,
    image: "/cell-organelles/vakuole.png",
    image1x1: "/cell-organelles/vakuole_1x1.png",
    topic: CBTopic.Zelle,
  },
  {
    id: "cytoskelett",
    term: "Cytoskelett",
    definition:
      "Das Cytoskelett besteht aus Proteinfasern und bildet ein Gerüst im Inneren der Zelle. Es verleiht der Zelle Struktur, Form und ermöglicht die Bewegung von Organellen. Es umfasst Mikrotubuli, Mikrofilamente und intermediäre Filamente..",
    unlockLevel: 1,
    topic: CBTopic.Zelle,
  },
  {
    id: "nukleolus",
    term: "Nukleolus (Kernkörperchen)",
    definition:
      "Das Kernkörperchen ist ein Bereich im Zellkern, der an der Synthese von ribosomaler RNA (rRNA) und der Zusammenbau von Ribosomen beteiligt ",
    unlockLevel: 1,
    topic: CBTopic.Zelle,
  },
  {
    id: "kernporen",
    term: "Kernporen",
    definition:
      "Kernporen sind komplexe Proteinstrukturen in der Kernmembran, die den Transport von Molekülen zwischen dem Zellkern und dem Zytoplasma ermöglichen.",
    unlockLevel: 1,
    topic: CBTopic.Zelle,
  },
  {
    id: "centriolen",
    term: "Centriolen",
    definition:
      "Centriolen sind zylinderförmige Strukturen, die eine wichtige Rolle bei der Zellteilung spielen. Sie sind auch an der Bildung von Zellfortsätzen wie Zilien und Flagellen beteiligt.",
    unlockLevel: 1,
    image: "/cell-organelles/centriolen.png",
    image1x1: "/cell-organelles/centriolen_1x1.png",
    topic: CBTopic.Zelle,
  },

  {
    id: "mitose",
    term: "Mitose",
    definition:
      "Die Mitose ist ein Prozess der Zellteilung, der in Körperzellen (somatischen Zellen) vorkommt. Sie besteht aus mehreren Phasen und führt zur Bildung von zwei nahezu genetisch identischen Tochterzellen. ",
    unlockLevel: 2,
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "interphase",
    term: "Interphase",
    definition:
      "Die längste Phase des Zellzyklus, in der die Zelle wächst, sich auf die Teilung vorbereitet und DNA repliziert.",
    unlockLevel: 2,
    image: "/cell-devision/mitosis/mitose-interphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "prophase",
    term: "Prophase",
    definition:
      "Die Chromosomen kondensieren, die Kernkörperchen und das Kernmembran lösen sich auf, und der Spindelapparat bildet sich.",
    unlockLevel: 2,
    image: "/cell-devision/mitosis/mitose-prophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "metaphase",
    term: "Metaphase",
    definition:
      "Die kondensierten Chromosomen ordnen sich an der Äquatorialebene - in der Mitte der Zelle - an und sind mit dem Spindelapparat verbunden.",
    unlockLevel: 2,
    image: "/cell-devision/mitosis/mitose-metaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "anaphase",
    term: "Anaphase",
    definition:
      "Die Schwesterchromatiden der Chromosomen werden getrennt und zu entgegengesetzten Polen der Zelle gezogen.",
    unlockLevel: 2,
    image: "/cell-devision/mitosis/mitose-anaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "telophase",
    term: "Telophase",
    definition:
      "Die Chromosomen entspiralisieren und werden zu einem neuen Satz von Chromosomen in den Tochterzellen. Das Kernkörperchen und die Kernmembran bilden sich aus.",
    unlockLevel: 2,
    image: "/cell-devision/mitosis/mitose-telophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "zytokinese-mitose",
    term: "Zytokinese-Mitose",
    definition:
      "Die Zytokinese findet zum Ende der Mitose statt. Dabei schnürt sich die Zellmembran ein, um die Zelle in zwei Tochterzellen zu teilen.",
    unlockLevel: 2,
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "meiose",
    term: "Meiose",
    definition:
      "Die Meiose ist ein Prozess der reduktiven Zellteilung, der in den Keimzellen stattfindet und zur Bildung von Gameten (Ei- und Spermazellen) führt. Im Gegensatz zur Mitose wird die Meiose in zwei aufeinanderfolgenden Teilungen durchgeführt und führt zur Bildung von vier haploiden Zellen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "meiose I",
    term: "Meiose I",
    definition:
      "Die erste Teilung der Meiose, bei der die homologen Chromosomen voneinander getrennt werden.",
    unlockLevel: 2,
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "prophase I",
    term: "Prophase I",
    definition:
      "Die längste Phase der Meiose I, in der Chromosomen kondensieren, Crossing-Over stattfindet und die Kernmembran sich auflöst.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-i-prophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "metaphase I",
    term: "Metaphase I",
    definition:
      "Die homologen Chromosomenpaare ordnen sich in der Äquatorialebene - in der Mitte der Zelle - an und sind mit dem Spindelapparat verbunden.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-i-metaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "anaphase I",
    term: "Anaphase I",
    definition:
      "Die homologen Chromosomen werden getrennt und zu entgegengesetzten Polen der Zelle gezogen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-i-anaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "telophase I",
    term: "Telophase I",
    definition:
      "Die Chromosomen entspiralisieren, das Kernkörperchen und die Kernmembran bilden sich aus, und es erfolgt eine vorläufige Aufteilung der Zelle.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-i-telophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "meiose II",
    term: "Meiose II",
    definition:
      "Die zweite Teilung der Meiose, bei der die Schwesterchromatiden getrennt werden.",
    unlockLevel: 2,
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "prophase II",
    term: "Prophase II",
    definition:
      "Diese Phasen ähneln denen der Mitose, wobei die Chromosomen der Schwesterchromatiden getrennt werden und vier haploide Zellen entstehen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-ii-prophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "metaphase II",
    term: "Metaphase II",
    definition:
      "Diese Phasen ähneln denen der Mitose, wobei die Chromosomen der Schwesterchromatiden getrennt werden und vier haploide Zellen entstehen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-ii-metaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "anaphase II",
    term: "Anaphase II",
    definition:
      "Diese Phasen ähneln denen der Mitose, wobei die Chromosomen der Schwesterchromatiden getrennt werden und vier haploide Zellen entstehen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-ii-anaphase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "telophase II",
    term: "Telophase II",
    definition:
      "Diese Phasen ähneln denen der Mitose, wobei die Chromosomen der Schwesterchromatiden getrennt werden und vier haploide Zellen entstehen.",
    unlockLevel: 2,
    image: "/cell-devision/meiosis/meiose-ii-telophase.png",
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "zytokinese-meiose",
    term: "Zytokinese Meiose",
    definition:
      "Diese Phasen ähneln denen der Mitose, wobei die Chromosomen der Schwesterchromatiden getrennt werden und vier haploide Zellen entstehen.",
    unlockLevel: 2,
    topic: CBTopic.MitoseMeiose,
  },
  {
    id: "nukleotide",
    term: "Nukleotide",
    definition:
      "Nukleotide sind die Bausteine der DNA. Sie bestehen aus einem Phosphatrest, einem Zucker (Desoxyribose) und einer stickstoffhaltigen Base.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "desoxyribose",
    term: "Desoxyribose",
    definition:
      "Desoxyribose ist ein Zucker, der in den Nukleotiden der DNA vorkommt. Es ist ein pentagonaler Ring, der mit Phosphat und einer Base verknüpft ist.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "phosphatgruppe",
    term: "Phosphatgruppe",
    definition:
      "Eine Phosphatgruppe ist ein Bestandteil des Nukleotids und besteht aus einem Phosphoratom, das mit Sauerstoffatomen verbunden ist. Sie verbindet die Zuckerreste benachbarter Nukleotide des DNA-Strangs.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "basen",
    term: "Basen",
    definition:
      "Es gibt vier verschiedene Basen in der DNA: Adenin (A), Thymin (T), Cytosin (C) und Guanin (G). Die komplementäre Basenpaarung zwischen den beiden DNA-Strängen erfolgt nach dem Watson-Crick-Modell: Adenin bindet an Thymin und Cytosin bindet an Guanin.",
    unlockLevel: 3,
    image: "/dna/basen.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "adenin",
    term: "Adenin (A)",
    definition: "Base in der DNA. Komplementäre Base: Thymin",
    unlockLevel: 3,
    image: "/dna/adenin.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "thymin",
    term: "Thymin (T)",
    definition: "Base in der DNA. Komplementäre Base: Adenin",
    unlockLevel: 3,
    image: "/dna/thymin.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "cytosin",
    term: "Cytosin (C)",
    definition: "Base in der DNA. Komplementäre Base: Guanin",
    unlockLevel: 3,
    image: "/dna/cytosin.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "guanin",
    term: "Guanin (G)",
    definition: "Base in der DNA. Komplementäre Base: Cytosin",
    unlockLevel: 3,
    image: "/dna/guanin.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "doppelhelix",
    term: "Doppelhelix",
    definition:
      "Die DNA hat eine charakteristische Struktur, die als Doppelhelix bezeichnet wird. Die beiden DNA-Stränge sind spiralförmig miteinander verbunden und winden sich um eine gemeinsame Achse.",
    unlockLevel: 3,
    image: "/dna/doppelhelix.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "basenpaarung",
    term: "Basenpaarung",
    definition:
      "Die Basenpaarung ist die spezifische Verbindung zwischen den Basen der beiden DNA-Stränge. Adenin (A) bindet über zwei Wasserstoffbrückenbindungen an Thymin (T), während Cytosin (C) über drei Wasserstoffbrückenbindungen an Guanin (G) bindet.",
    unlockLevel: 3,
    image: "/dna/basenpaarung.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "antiparalleler Strang",
    term: "Antiparalleler Strang",
    definition:
      "Die beiden DNA-Stränge in der Doppelhelix laufen in entgegengesetzter Richtung. Einer der Stränge wird als 5'-Strang bezeichnet, da er in Richtung des 5'-Endes des Zuckers zeigt, während der andere Strang als 3'-Strang bezeichnet wird.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "gen",
    term: "Gen",
    definition:
      "Ein Gen ist eine Sequenz der DNA, die die Informationen für den Aufbau eines spezifischen Proteins oder RNA-Moleküls enthält. Gene tragen die genetische Information, die für die Vererbung von Merkmalen und die Regulation von zellulären Prozessen verantwortlich ist.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "chromosom",
    term: "Chromosom",
    definition:
      "Chromosomen sind eine strukturierte Form der DNA, die die gesamte genetische Information eines Organismus enthalten und sich im Zellkern befinden. Chromosomen bestehen aus einer langen DNA-Sequenz, die mit Hilfe von Proteinen (=Histonen) verpackt und kondensiert ist.",
    unlockLevel: 3,
    image: "/dna/chromosom.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "homologe Chromosome",
    term: "Homologe Chromosomen",
    definition:
      "Homologe Chromosomen sind ein Paar von Chromosomen, die ähnliche genetische Informationen tragen. Ein Chromosom stammt von der Mutter und das andere vom Vater.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "autosome",
    term: "Autosome",
    definition:
      "Chromosomen sind eine strukturierte Form der DNA, die die gesamte genetische Information eines Organismus enthalten und sich im Zellkern befinden. Chromosomen bestehen aus einer langen DNA-Sequenz, die mit Hilfe von Proteinen (=Histonen) verpackt und kondensiert ist.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "diploid",
    term: "Diploid",
    definition:
      "Autosomen sind Chromosomen, die nicht geschlechtsbestimmend sind. Ein Mensch hat 22 Autosomen-Paare.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "geschlechtschromosomen",
    term: "Geschlechtschromosomen",
    definition:
      "Die Geschlechtschromosomen bestimmen das Geschlecht eines Menschen. Beim Menschen bestehen sie aus einem Paar von X- und Y-Chromosomen bei Männern (XY) und einem Paar von X-Chromosomen bei Frauen (XX).",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "karyogramm",
    term: "Karyogramm",
    definition:
      "Das Karyogramm ist das Gesamtbild der Chromosomenanordnung eines Individuums. Es repräsentiert die Anzahl, Form und Größe der Chromosomen eines Organismus.",
    unlockLevel: 3,
    image: "/dna/karyogramm.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "trisomie",
    term: "Trisomie",
    definition:
      "Trisomie ist eine genetische Abnormalität, bei der ein zusätzliches Chromosom zu den normalen zwei Exemplaren in einer Zelle vorliegt. Ein bekanntes Beispiel ist das Down-Syndrom (Trisomie 21), bei dem es ein zusätzliches Chromosom 21 gibt.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "deletion",
    term: "Deletion",
    definition:
      "Eine Deletion tritt auf, wenn ein Stück eines Chromosoms fehlt. Es kann zu genetischen Störungen führen, wenn wichtige genetische Informationen gelöscht werden.",
    unlockLevel: 3,
    image: "/dna/deletion.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "duplikation",
    term: "Duplikation",
    definition:
      "Eine Duplikation tritt auf, wenn ein Stück eines Chromosoms mehrfach vorhanden ist. Es kann zu genetischen Veränderungen führen, wenn zusätzliche genetische Informationen vorhanden sind.",
    unlockLevel: 3,
    image: "/dna/duplikation.png",
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "translokation",
    term: "Translokation",
    definition:
      "Eine Translokation ist eine chromosomale Anomalie, bei der Teile von zwei nicht homologen Chromosomen ausgetauscht werden. Es kann zu genetischen Störungen führen, wenn wichtige genetische Informationen in die falschen Chromosomenregionen gelangen.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "dominanz",
    term: "Dominanz",
    definition:
      "Nach der Mendelschen Regel der Dominanz hat ein Allel in einer Allelpaarung (Genotyp) die Fähigkeit, sich gegenüber einem anderen Allel durchzusetzen und das Erscheinungsbild (Phänotyp) zu bestimmen. Das dominante Allel wird mit einem Großbuchstaben gekennzeichnet, während das rezessive Allel mit einem Kleinbuchstaben gekennzeichnet wird.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "rezessivität",
    term: "Rezessivität",
    definition:
      "Rezessive Allele werden von dominanten Allelen überdeckt und zeigen ihren Phänotyp nur, wenn sie in doppelter Ausführung vorliegen. Rezessive Allele werden mit einem Kleinbuchstaben gekennzeichnet.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "aufspaltung",
    term: "Aufspaltung",
    definition:
      "Die Mendelsche Regel der Aufspaltung besagt, dass die Allele eines Gens während der Bildung von Gameten (Keimzellen) voneinander getrennt werden. Jedes Allel wird in einem separaten Gameten weitergegeben.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "genotyp",
    term: "Genotyp",
    definition:
      "Der Genotyp bezieht sich auf die genetische Zusammensetzung eines Organismus. Es umfasst die Kombination der Allele, die ein Individuum für ein bestimmtes Merkmal trägt.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "phänotyp",
    term: "Phänotyp",
    definition:
      "Der Phänotyp bezieht sich auf das beobachtbare Erscheinungsbild eines Organismus. Er wird durch die Wechselwirkung zwischen den Allelen eines Gens und Umweltfaktoren bestimmt.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "monohybride kreuzung",
    term: "Monohybride Kreuzung",
    definition:
      "Eine monohybride Kreuzung bezieht sich auf die Kreuzung von Individuen, die sich nur in einem einzigen Merkmal (genetisches Merkmal) unterscheiden. Mendel untersuchte in seinen Experimenten monohybride Kreuzungen.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "dihybride kreuzung",
    term: "Dihybride Kreuzung",
    definition:
      "Eine dihybride Kreuzung bezieht sich auf die Kreuzung von Individuen, die sich in zwei verschiedenen Merkmalen (genetische Merkmale) unterscheiden. Mendel untersuchte auch dihybride Kreuzungen.",
    unlockLevel: 3,
    topic: CBTopic.AufbauDNA,
  },
  {
    id: "ein-chromatid-chromosomen",
    term: "Ein-Chromatid-Chromosomen",
    definition:
      "Ein-Chromatid-Chromosomen sind Chromosomen, die aus einem einzigen Chromatid bestehen. Ein Chromatid ist ein einzelner DNA-Strang, der während der Zellteilung repliziert wird und beide durch ein Centromer verbunden werden.",
    unlockLevel: 3,
    image: "/dna/chromatid.png",
    topic: CBTopic.AufbauDNA,
  },
];
